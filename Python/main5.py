...
\brief This file contains the execution to thesis
...
class Structure:
    ...
    def updateFovea( self, index, params ):
        self.indexFovea = index
...
def filtered( value, parameters, index, multifovea):
...
def main():
    ...
    if ( len( arguments ) != 3 ):
        ...
        # trackbar to filter
        cv2.createTrackbar( 'filter', winname, 0, 100, partial(filtered, parameters=params, index=index, multifovea=multifovea) )
        ...
        while ( True ):
            ...
            if ( key == ord('m') ):
                ...
                structure.updateFovea( index, params )
            if ( key == ord('i') ):
                statistics.displayConfigurations( model, scene, params, [-1, -1] )
                print("w = "+str(params.w[0])+", s = "+str(params.w[1]) )
            if ( key == ord('g') ):
                statistics.plotProportion( model, scene, multifovea.foveas[index], params )
                statistics.plotFunction( model, scene, multifovea.foveas[index], params )
